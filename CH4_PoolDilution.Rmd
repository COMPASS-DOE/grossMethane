...
for(i in unique(incdat$id)) {
    ...
    incdat %>%
        ...
        select(id, round, vol, time_days,
               ...
               AP_obs, Nm, Nd) ->
        dat
    # Estimate starting k by slope of 13C following para. 21:
...
incdat_out %>%
    ...
    xlab("\n Timestep") + ylab("(13C-CH4/Total CH4) x 100 \n") +
    ggtitle("Atom% 13C") + theme(legend.position = "bottom")
...
incdat_out %>%
    ...
    scale_color_discrete(guide = "none") +
    facet_wrap(~as.numeric(id), scales = "free") +
    xlab("\n Timestep") + ylab("Volume (mL) \n") +
    ggtitle("Total Methane") + theme(legend.position = "bottom")
```
...
incdat_out %>%
    filter(id %in% c("2","31","52","61","64","71"),
           round == "T1") %>%
    select(id, P, k, Pt, Ct, AP_obs, AP_pred) %>%
    mutate(across(where(is.numeric), ~ num(., digits = 2))) -> results

kable(results,
      caption = "<center><strong>What the heck are the units here?</strong></center>",
      escape = FALSE,
      format = "html") %>%
