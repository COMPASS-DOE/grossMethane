...
The model optimizes the solutions by weighting information against the signal:noise ratio of concentration and 13C-signature using instrument precision and measurement errors.
...
for(i in unique(incdat$id)) {
    ...
    incdat %>%
        filter(id == i) %>%
        select(id, round, vol, time_days, cal12CH4ml, cal13CH4ml, AP_obs, Nm, Nd) ->
        dat
